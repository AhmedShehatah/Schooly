...
class AddPostWidget extends StatefulWidget {
  const AddPostWidget({Key? key, required this.classRoomId}) : super(key: key);
...
class _AddPostWidgetState extends State<AddPostWidget> {
  ...
  Widget build(BuildContext context) {
    return BlocBuilder<LoginCubit, BaseState<Auth>>(
      bloc: sl<LoginCubit>(),
      builder: (context, state) {
        final data = state.whenOrNull(success: (auth) => auth);
        return Scaffold(
          backgroundColor: Palette.character.primaryInverse,
          resizeToAvoidBottomInset: false,
          appBar: AppBar(
            title: CustomText.s16(lz.schooly),
            leading: IconButton(
